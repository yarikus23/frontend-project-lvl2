...
test('adding values', () => {
  const obj1 = parser(pathBuilder('plain-add1.json'));
  const obj2 = parser(pathBuilder('plain-add2.json'));
  const etalon = "Property 'setting4' was added with value: 'blah blah'";
  expect(plainer(genDiff(obj1, obj2))).toEqual(etalon);
});

test('removing values', () => {
  const obj1 = parser(pathBuilder('plain-remove1.json'));
  const obj2 = parser(pathBuilder('plain-remove2.json'));
  const etalon = "Property 'setting3' was removed";
  expect(plainer(genDiff(obj1, obj2))).toEqual(etalon);
});

test('updating values', () => {
  const obj1 = parser(pathBuilder('plain-update1.json'));
  const obj2 = parser(pathBuilder('plain-update2.json'));
  const etalon = "Property 'nest' was updated. From [complex value] to null";
  expect(plainer(genDiff(obj1, obj2))).toEqual(etalon);
});

test('unchange and path of keys', () => {
  const obj1 = parser(pathBuilder('plain-unchange1.json'));
  const obj2 = parser(pathBuilder('plain-unchange2.json'));
